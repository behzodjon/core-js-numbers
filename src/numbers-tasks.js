...
function getAverage(value1, value2) {
  ...
  return Number.isFinite(sum) ? sum / 2 : sum;
}
 ...
 */
function isPrime(n) {
  return Array.from({ length: n - 1 }, (_, i) => i + 2).every(
    (num) => n % num !== 0
  );
}
...
function getFibonacciNumber(index) {
  return Array.from({ length: index + 1 }, (_, i) => i).reduce(
    (acc, num, i) => (i > 1 ? acc + num : acc),
    0
  );
}
...
function getSumOfDigits(num) {
  return num
    .toString()
    .split('')
    .reduce((acc, num1) => acc + Number.parseInt(num1, 10), 0);
}
...
function isPowerOfTwo(num) {
  return (
    num
      .toString(2)
      .split('')
      .reduce((acc, num1) => acc + Number.parseInt(num1, 10), 0) === 1
  );
}
...
function getCountOfOddNumbers(number) {
  return Array.from({ length: number + 1 }, (_, i) => i).filter(
    (num) => num % 2 !== 0
  ).length;
}
